...
class HeatmapGenerator:
    ...
    def __init__(self, model, method_type = "2D", target_layer=None, save_path_folder = "results"):
        ...
        assert isinstance(model, Module), "Model should be an instance of torch.nn.Module."
    ...
    def generate_heatmap(self, input_image, method='integrated_gradients', attr_type= "2D", num_channels = "3", target_val = 0):
        """
        ...
        assert isinstance(input_image, torch.Tensor), "Input image should be a PyTorch Tensor."
        assert len(input_image.shape) in [4], "Input image should have 4 (1, C, H, W) dimensions."
        ...
        if method == 'integrated_gradients':
            attributions = self.integrated_gradients.attribute(input_image, target=target, baselines=input_image * 0)
        ...
        attributions = interpolate(attributions, size=input_image.shape[2:], mode='bilinear', align_corners=False)
    ...
    def visualize_attributions(self, attributions, original_image=None, method='method_name', percentile=98, alpha=0.5, image_name=""):
            """
            ...
            if len(attributions.shape) == 3:
                if self.method_type == "2D":
                    ...
                    # Grid dimensions
                    grid_dims = int(np.ceil(np.sqrt(original_image.shape[2])))
                    ...
                    for i in range(grid_dims):
                        for j in range(grid_dims):
                            ...
                            if slice_index < original_image.shape[2]:
                                attr_slice = attributions[:, :, slice_index]
                                threshold = self._cumulative_sum_threshold(np.abs(attr_slice), percentile)
                                attributions_norm = self._normalize_scale(attr_slice, threshold)
                                ax.imshow(attributions_norm, cmap=self.default_cmap, vmin=-1, vmax=1)
                    ...
                    for i in range(grid_dims):
                        for j in range(grid_dims):
                            ...
                            if slice_index < original_image.shape[2]:
                                ...
                                im = ax.imshow(attributions_norm, cmap=self.default_cmap, vmin=-1, vmax=1, alpha=1-alpha)
